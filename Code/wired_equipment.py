...
def create_or_update_wired_equipment(user_name, user_password):
    ...
    try:
        ...
        if result:  # 장비가 존재하는 경우
            ...
            cursor.execute("UPDATE wired_equipment SET quantity = %s WHERE id = %s;", (new_quantity, equipment_id))
            print(f"{name} 장비의 수량이 {existing_quantity}에서 {new_quantity}로 업데이트되었습니다.")
        else:  # 장비가 존재하지 않는 경우
            cursor.execute("INSERT INTO wired_equipment (name, quantity) VALUES (%s, %s) RETURNING id;", (name, quantity))
...
def update_wired_equipment_quantity(user_name, user_password, equipment_id=None, delta_quantity=None):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE wired_equipment SET quantity = quantity + %s WHERE id = %s;", (delta_quantity, equipment_id))
        
        # 업데이트 후 현재 수량 조회
        cursor.execute("SELECT name, quantity FROM wired_equipment WHERE id = %s;", (equipment_id,))
...
# 유선 장비 삭제 및 입력 처리 함수
def delete_wired_equipment(user_name, user_password):
    ...
    if equipment:
        ...
        try:
            ...
            cursor.execute("UPDATE wired_equipment SET id = id - 1 WHERE id > %s;", (equipment_id,))
            cursor.execute("SELECT setval('wired_equipment_id_seq', (SELECT MAX(id) FROM wired_equipment));")
            conn.commit()  # 변경 사항 커밋
...
def manage_messages(user_name, user_password, user_department):
    while True:
        messages = perform_crud_operations(user_name, user_password, 'read_messages')  # 메시지 목록 조회
        ...
        if sub_choice == '1':  # 메시지 보내기
            ...
            # 삭제 가능한 메시지 조회
            delete_messages = perform_crud_operations(user_name, user_password, 'read_messages')  # 메시지 목록 조회
...
def perform_crud_operations(user_name, user_password, operation, *args):
    ...
    try:
        if operation == 'create':
            create_or_update_wired_equipment(cursor, *args)
        elif operation == 'search':
            result = search_wired_equipment(cursor, *args)
        elif operation == 'update_quantity':
            update_wired_equipment_quantity(user_name, user_password, *args)
        elif operation == 'delete':
            delete_wired_equipment(cursor, *args)
        elif operation == 'read_incidents':
            result = read_incidents(cursor)
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            create_or_update_wired_equipment(user_name, user_password)  # 장비 생성 또는 업데이트
        elif choice == '2':
            handle_search_wired_equipment(user_name, user_password)  # 장비 목록 조회
        elif choice == '3':
            update_wired_equipment_quantity(user_name, user_password)  # 장비 수량 업데이트
        elif choice == '4':
            delete_wired_equipment(user_name, user_password)  # 장비 삭제
        elif choice == '5':
