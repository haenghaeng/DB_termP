...
def read_all_incidents(cursor):
    cursor.execute('''
                   SELECT id, contact, details, wired_tool_name, wired_tool_quantity, department
                   FROM incident_reports
                   WHERE wired_tool_quantity > 0;
                   ''')
        ...
        if incident_list:
            ...
            try:
                ...
                if incident_id in [incident[0] for incident in incident_list]:
                    cursor.execute('''
                                   SELECT contact, details, wired_tool_name, wired_tool_quantity, department
                                   FROM incident_reports
                                   WHERE id = %s;
                                   ''', (incident_id,))
