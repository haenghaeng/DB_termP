...
def create_or_update_wireless_equipment(cursor, name, quantity):
    ...
    if result:  # 장비가 존재하는 경우
        ...
        cursor.execute("SELECT id FROM wireless_equipment ORDER BY id ASC;")
        ids = cursor.fetchall()
        available_id = None
        ...
        cursor.execute("""
            INSERT INTO wireless_equipment (id, name, quantity) VALUES (%s, %s, %s);
        """, (available_id, name, quantity))
        print("장비가 생성되었습니다.")

# 무선 장비 조회 함수
def read_wireless_equipment(cursor):
    cursor.execute("SELECT * FROM wireless_equipment ORDER BY id ASC;")  # ID 기준으로 오름차순 정렬
    return cursor.fetchall()  # 모든 장비 데이터 반환

# 무선 장비 수량 업데이트 함수
def update_wireless_equipment_quantity(cursor, equipment_id, quantity):
    cursor.execute("""
        UPDATE wireless_equipment SET quantity = %s WHERE id = %s;  # 특정 ID의 수량 업데이트
...
def perform_crud_operations(user_name, user_password, operation, *args):
    ...
    if operation == 'create':
        ...
        return read_wireless_equipment(cursor)  # 장비 목록 조회
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            equipment_list = perform_crud_operations(user_name, user_password, 'read')  # 장비 목록 조회
            print("현재 무선 장비 목록:")
            ...
            perform_crud_operations(user_name, user_password, 'update_quantity', equipment_id, quantity)  # 장비 수량 업데이트
