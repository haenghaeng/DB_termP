...
def create_or_update_wireless_equipment(user_name, user_password):
    ...
    try:
        ...
        if result:  # 장비가 존재하는 경우
            ...
            cursor.execute("UPDATE wireless_equipment SET quantity = %s WHERE id = %s;", (new_quantity, equipment_id))
            print(f"{name} 장비의 수량이 {existing_quantity}에서 {new_quantity}로 업데이트되었습니다.")
        else:  # 장비가 존재하지 않는 경우
            cursor.execute("INSERT INTO wireless_equipment (name, quantity) VALUES (%s, %s) RETURNING id;", (name, quantity))
...
# 장비 목록 조회 함수
def handle_search_wireless_equipment(user_name, user_password):
...
def update_wireless_equipment_quantity(user_name, user_password, equipment_id=None, delta_quantity=None):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE wireless_equipment SET quantity = quantity + %s WHERE id = %s;", (delta_quantity, equipment_id))
        
        # 업데이트 후 현재 수량 조회
        cursor.execute("SELECT name, quantity FROM wireless_equipment WHERE id = %s;", (equipment_id,))
...
# 무선 장비 삭제 및 입력 처리 함수
def delete_wireless_equipment(user_name, user_password):
    ...
    if equipment:
        ...
        try:
            ...
            cursor.execute("UPDATE wireless_equipment SET id = id - 1 WHERE id > %s;", (equipment_id,))
            cursor.execute("SELECT setval('wireless_equipment_id_seq', (SELECT MAX(id) FROM wireless_equipment));")
            conn.commit()  # 변경 사항 커밋
...
# 장비 사용 함수
def handle_use_wireless_equipment(user_name, user_password):
    conn = try_connection(user_name, user_password)
    cursor = conn.cursor()
    try:
            ...
            if message_content:
                try:
                    cursor.execute("""
                        ...
                        VALUES (%s, %s);
                    """, (user_department, message_content))
                    conn.commit()
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            create_or_update_wireless_equipment(user_name, user_password)  # 장비 생성 또는 업데이트
        elif choice == '2':
            handle_search_wireless_equipment(user_name, user_password)  # 장비 목록 조회
        elif choice == '3':
            update_wireless_equipment_quantity(user_name, user_password)  # 장비 수량 업데이트
        elif choice == '4':
            delete_wireless_equipment(user_name, user_password)  # 장비 삭제
        elif choice == '5':
            handle_use_wireless_equipment(user_name, user_password)  # 장비 사용
        elif choice == '6':
