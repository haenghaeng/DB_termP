...
def create_or_update_computer_equipment(cursor, name, quantity):
    try:
        ...
        if result:  # 장비가 존재하는 경우
            ...
            cursor.execute("""
                UPDATE computer_equipment SET quantity = %s WHERE id = %s;
            """, (new_quantity, equipment_id))
            print(f"{name} 장비의 수량이 {existing_quantity}에서 {new_quantity}로 업데이트되었습니다.")
        else:  # 장비가 존재하지 않는 경우
            cursor.execute("""
                INSERT INTO computer_equipment (name, quantity) VALUES (%s, %s)
                           RETURNING id;
                """, (name, quantity))
...
def delete_computer_equipment(cursor, equipment_id):
    ...
    cursor.execute("""
        UPDATE computer_equipment SET id = id - 1 WHERE id > %s;
    """, (equipment_id,))
    cursor.execute("""
        SELECT setval('computer_equipment_id_seq', (SELECT MAX(id) FROM computer_equipment));
    """)
...
# 유선 장비 사용 함수
def use_computer_equipment(cursor, incident_id):
